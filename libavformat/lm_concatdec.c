...
{
    return memcmp(probe->buf, "lm_concat version 1.0", 20) ?
           0 : AVPROBE_SCORE_MAX;
}
...
static void * get_a_ptr(char *vdata_one_path, char *vdata_db_path)
{
...
#ifdef O_BINARY
    access |= O_BINARY; // linux 没有 O_BINARY
#endif

	//int fd = avpriv_open("vdata.one", O_RDONLY | O_BINARY, 0666);
	int fd = avpriv_open(vdata_one_path, access, 0666);
	av_log(NULL, AV_LOG_ERROR, "get_a_ptr(): fd = %d\n", fd);
	ret->fd = fd;
...
static int concat_read_header(AVFormatContext *avf)
{
    ...
    while (1) {
        ...
        if (!strcmp(keyword, "file")) {
            ...
            char *filename = my_get_token((const char **)&cursor, SPACE_CHARS);
            if (!filename) {
			...
			strcat(filename, "_");
			strcat(filename, cat->strPtr);
			av_log(avf, AV_LOG_ERROR, "Line %d: filename = %s\n", line, filename);
        ...
        } else if (!strcmp(keyword, "lm_concat")) {
...
AVInputFormat ff_lm_concat_demuxer = {
    .name           = "lm_concat",
